# =============================================================================
# AWS EKS Module - NEXUS Platform
# Replaces: Azure Kubernetes Service (modules/aks)
# =============================================================================
...
# Secrets Manager access for nodes
resource "aws_iam_role_policy" "node_secrets_access" {
  name = "${local.cluster_name}-node-secrets-policy"
    ...
    Statement = [
      {
        ...
        Effect   = "Allow"
        Resource = "arn:${data.aws_partition.current.partition}:secretsmanager:*:${data.aws_caller_identity.current.account_id}:secret:${var.project}/*"
      }
    ]
  })
}
...
resource "aws_eks_node_group" "worker" {
  count = var.worker_max_size > 0 ? 1 : 0
